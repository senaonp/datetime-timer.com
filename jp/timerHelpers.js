...
var validateFields = function() {
	var errors = [];
	if (!nowStart) {
		if (elemSelector("#startDateCustom").value == "") { errors.push(" エラー 「自定の開始日付が有効でありません」 "); };
		if (elemSelector("#startTimeCustom").value == "") { errors.push(" エラー 「自定の開始時間が有効でありません」 "); };
	}
	if (elemSelector("#endDate").value == "") { errors.push(" エラー 「終了日付が有効でありません」 "); };
	if (elemSelector("#endTime").value == "") { errors.push(" エラー 「終了時間が有効でありません」 "); };
...
var url = "https://datetime-timer.com/jp/?";
var nowStart = true;
